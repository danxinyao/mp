...
function showTips( tips, height, time ){
    ...
    $( 'div.tipsClass' ).css({
        ...
        'opacity' : 'http://meipin365.cn/data/resource/js/0.90',
        'z-index' : '9999'
    }).show();
...
function collect_store(fav_id,jstype,jsobj){
    $.get('http://meipin365.cn/data/resource/js/index.php?act=index&op=login', function(result){
        if(result=='0'){
            login_dialog();
        }else{
            var url = 'http://meipin365.cn/data/resource/js/index.php?act=member_favorite_store&op=favoritestore';
...
function collect_goods(fav_id,jstype,jsobj){
    $.get('http://meipin365.cn/data/resource/js/index.php?act=index&op=login', function(result){
        if(result=='0'){
            ...
            var url = 'http://meipin365.cn/data/resource/js/index.php?act=member_favorite_goods&op=favoritegoods';
            $.getJSON(url, {'fid':fav_id}, function(data){
                if (data.done)
            {
                showDialog(data.msg, 'succ','','','','','','','','',2);
...
(function($) {
    $.show_nc_login = function(options) {
        var settings = $.extend({}, {action:'http://meipin365.cn/index.php?act=login&op=login&inajax=1', nchash:'', formhash:'' ,anchor:''}, options);
	                ...
	                ajax: {
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		if(data.qq != ''){
		                			_info += '<a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=%27+data.qq+%27&site=qq&menu=yes" title="QQ: '+data.qq+'"><img border="0" src="http://wpa.qq.com/pa?p=2:'+data.qq+':52" style=" vertical-align: middle;"/></a>';
		                		}
		                		if(data.ww != ''){
			                    ...
			                    this.set('http://meipin365.cn/data/resource/js/content.text', ' ');this.set('http://meipin365.cn/data/resource/js/content.text', _content);
		                	}
		                }
	                }
	            },
...
function addcart(goods_id,quantity,callbackfunc) {
    ...
    var url = 'http://meipin365.cn/data/resource/js/index.php?act=cart&op=add';
